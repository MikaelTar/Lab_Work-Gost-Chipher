...
def gost_encrypt(data, key):
    ...
    for i in range(0, len(data), BLOCK_SIZE):
        ...
        encrypted_block = gost_encrypted_block(block_int, key)
        encrypted_data += encrypted_block.to_bytes(BLOCK_SIZE, byteorder='big')
    return encrypted_data

def gost_decrypt(data, key):
    key = struct.unpack('>8I', key)
    decrypted_data = b''
    for i in range(0, len(data), BLOCK_SIZE):
        block = data[i:i + BLOCK_SIZE]
        block_int = int.from_bytes(block, byteorder='big')
        decrypted_block = gost_decrypted_block(block_int, key)
